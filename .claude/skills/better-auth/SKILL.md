...
```bash
# Backend (NestJS)
pnpm install better-auth @auth/drizzle-adapter drizzle-orm pg
...
1. **Install dependencies**
   ...
   ```bash
   pnpm install drizzle-orm pg @auth/drizzle-adapter better-auth
   pnpm install -D drizzle-kit
   ```
...
8. **Type Safety**: Use `pnpx better-auth typegen` for full TypeScript coverage
